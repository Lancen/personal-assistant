...
export default function NotesPage() {
  ...
  async function loadNotes() {
    try {
      const res = await api.get<{ success: boolean; data: Note[] }>('/api/notes');
  ...
  async function loadTags() {
    try {
      const res = await api.get<{ success: boolean; data: string[] }>('/api/notes/tags');
      if (res.success && res.data) {
        setTags(res.data);
      }
    } catch (error) {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await api.delete<{ success: boolean }>(`/api/notes/${id}`);
  ...
  async function togglePin(id: string) {
    try {
      const res = await api.patch<{ success: boolean; data: Note }>(`/api/notes/${id}/toggle-pin`);
      if (res.success && res.data) {
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        // 编辑
        const res = await api.patch<{ success: boolean; data: Note }>(`/api/notes/${editingId}`, {
        ...
        // 新建
        const res = await api.post<{ success: boolean; data: Note }>('/api/notes', {
          title: currentNote.title,
          content: currentNote.content,
          tags: tagsArray,
          isPinned: currentNote.isPinned,
