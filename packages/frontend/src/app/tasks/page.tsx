...
import { Task, Quadrant, CreateTaskRequest, UpdateTaskRequest, TaskStats } from '@personal-assistant/types';
...
export default function TasksPage() {
  ...
  async function loadTasks() {
    try {
      const res = await api.get<{ success: boolean; data: Task[]; pagination: any }>('/api/tasks');
  ...
  async function loadStats() {
    try {
      const res = await api.get<{ success: boolean; data: TaskStats }>('/api/tasks/stats');
      if (res.success && res.data) {
        setStats(res.data);
      }
    } catch (error) {
  ...
  const toggleTaskCompleted = async (id: string) => {
    try {
      const res = await api.patch<{ success: boolean; data: Task }>(`/api/tasks/${id}/toggle-completed`);
      if (res.success && res.data) {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post<{ success: boolean; data: Task }>('/api/tasks', newTask);
      if (res.success && res.data) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              {stats?.completed ?? 0} / {stats?.total ?? 0} 已完成 · {stats?.pending ?? 0} 待处理
            </p>
          </div>
          <button
