...
export default function NewEmotionPage() {
  ...
  async function handleSubmit(data: any) {
    try {
      const res = await api.emotion.records.create(data);
      if (res.success) {
        router.push('/emotion');
      }
    } catch {
      ...
      <div className="flex-1 flex flex-col min-h-dvh">
        <header className="h-16 border-b border-border bg-card/80 backdrop-blur sticky top-0 z-30">
          <div className="flex items-center justify-between h-full px-4 lg:px-6">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-semibold text-foreground">新建情绪记录</h1>
            </div>
        ...
        <main className="flex-1 p-4 lg:p-6">
          <div className="max-w-2xl mx-auto soft-card p-6">
            <EmotionRecordForm onSubmit={handleSubmit} submitLabel="保存记录" />
          </div>
        </main>
